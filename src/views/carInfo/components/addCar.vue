<template>
  <el-dialog title="新增车辆" :visible.sync="dialogVisible" width="50%" :before-close="closeModel">
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="addCar('car')">确 定</el-button>
    </span>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    /** 关闭 model 窗口 */
    closeModel() {
      this.$emit("addCarToCarInfo", "closeAddCarModel");
    },
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.axios({
            ...
            if (res.data.success) {
              //返回数据正常
              this.$message.success("加入车辆成功！");
              /** 重置弹窗内的数据 */
              this.resetData();
              /** 刷新 Table 数据 */
              this.$emit("addCarToCarInfo", "regainData");
              /** 关闭对话框 */
              this.closeModel();
            } else {
